""" Shared function for plotting a coupling map with protein domain annotations on the axes """

using PyCall
@isdefined(mpatches) || (const mpatches = pyimport("matplotlib.patches"))
@isdefined(mpatheffects) || (const mpatheffects = pyimport("matplotlib.patheffects"))
...
function plot_coup_map(Jmat::Vector{Vector{Union{Float64,Vector{Int64}}}};
    ...
    # Extract data in reverse order so highest-valued couplings are drawn on top
    ...
    ax_x_ticks.set_xticklabels(collect(0:tick_step:L), fontsize=12)
    ...
    ax_y_ticks.set_yticklabels(collect(0:tick_step:L), fontsize=12)
