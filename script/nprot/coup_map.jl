...
function plot_coup_map(Jmat::Vector{Vector{Union{Float64,Vector{Int64}}}};
    L::Int=421,
    idx_max::Int=75,
    doms_label::Vector{S}=["NTD", "SR", "CTD", "Tetramerization"],
    doms_edges::Vector{UnitRange{Int}}=[48:174, 176:206, 247:364, 400:419],
    doms_col::Vector{S}=["cyan", "yellow", "red", "green"],
    ...
    ax_x_ticks.set_xticks(collect(0:40:L))
    ax_x_ticks.set_xticklabels(collect(0:40:L), fontsize=12)
    ...
    ax_y_ticks.set_yticks(collect(0:40:L))
    ax_y_ticks.set_yticklabels(collect(0:40:L), fontsize=12)
...
fig, ax = plot_coup_map(Jmat; idx_max=75) # only plot top 75 couplings
